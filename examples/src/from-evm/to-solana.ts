/**
 * EVM → Solana transfer via Hyperlane + Hub forwarding
 *
 * Sends tokens from an EVM chain (Ethereum/Base/BSC) to Solana
 * via Hyperlane warp routes + Hub's forward module.
 * Uses the high-level transfer() API.
 *
 * Flow: EVM → Hyperlane → Hub → Hyperlane → Solana
 */
...
async function main() {
  ...
  console.log('EVM transaction confirmed. Tokens will be forwarded via Hub → Hyperlane → Solana.');
}
